    ...
    return (
        <div className="absolute inset-0 w-full h-full font-sans overflow-hidden bg-white text-slate-900 select-none" style={backgroundStyle}>
            ...
            <div className="relative w-full h-full flex flex-col z-10">
                ...
                {/* Header Section */}
                <div className="flex-none pt-20 pb-6 px-6 text-center z-20">
                ...
                <div className={`flex-1 min-h-0 w-full px-4 pb-4 ${style === 'carousel' ? 'flex flex-col justify-center' : 'overflow-y-auto no-scrollbar'}`}>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {images.map((img: any, idx: number) => (
                                    <button
                                        ...
                                        className="aspect-square bg-slate-100 rounded-2xl overflow-hidden shadow-sm relative hover:opacity-90 transition-opacity w-full border-none p-0 cursor-pointer"
                                    >
                                        <img
                                            src={img.url}
                    ...
                    {style === 'carousel' && (
                        <div className="w-full py-4">
                            ...
                            <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-24 pt-12 -mx-4 px-8 scrollbar-hide items-center">
                                {images.map((img: any, idx: number) => (
                                    <div
                                        key={idx}
                ...
                {selectedImage && (
                    <div
                        className="absolute inset-0 z-50 flex items-center justify-center bg-black/95 animate-in fade-in duration-200"
                        onClick={() => setSelectedImage(null)}
                    >
                        <button
                            type="button"
                            className="absolute top-6 right-6 p-2 bg-white/10 rounded-full text-white hover:bg-white/20 transition-colors backdrop-blur-md"
                            onClick={() => setSelectedImage(null)}
                        >
                            <X className="w-6 h-6" />
                        </button>
                        ...
                        <div className="w-full px-4 flex flex-col items-center">
                            ...
                            {selectedImage.caption && (
                                <p className="text-white text-center font-medium mt-4 max-w-[80%] text-sm">
                                    {selectedImage.caption}
                                </p>
                            )}
                        </div>
