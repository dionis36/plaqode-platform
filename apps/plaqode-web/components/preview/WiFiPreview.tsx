...
export function WiFiPreview({ data }: WiFiPreviewProps) {
    ...
    // Use empty defaults if data is missing (should not happen in viewer)
    const activeData = data || {};

    const wifi = {
        ssid: activeData?.wifi_details?.ssid || '',
        password: activeData?.wifi_details?.password || '',
        security: activeData?.wifi_details?.security || 'WPA2',
        hidden: activeData?.wifi_details?.hidden || false,
    };

    const network = {
        title: activeData?.network_info?.title || '',
        description: activeData?.network_info?.description || '',
        logo: activeData?.network_info?.logo || null,
    };
    ...
    const gradientType = styles.gradient_type;
    const gradientAngle = styles.gradient_angle;

    // Helper to lighten a color
    const lightenColor = (hex: string, percent: number = 30) => {
        if (!hex) return '#ffffff';
        // Basic hex validation/fallback
    ...
    return (
        <div className="flex-1 w-full h-full flex flex-col overflow-hidden relative" style={getBackgroundStyle()}>
            <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 space-y-6">
                ...
                >
                    ...
                    >
                        ...
                        {wifi.password ? 'Copy Password' : 'Connect'}
                    </button>
                </div>
            </div>
