...
export function URLPreview({ data }: URLPreviewProps) {
    ...
    const urlDetails = {
        destination_url: activeData?.url_details?.destination_url || '',
        title: activeData?.url_details?.title || 'External Link',
        description: activeData?.url_details?.description || '',
        logo: activeData?.url_details?.logo || null,
    };

    const redirectSettings = activeData?.redirect_settings || { delay: 3, show_preview: true, custom_message: '' };
