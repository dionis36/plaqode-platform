...
const PlatformIcons = {
    ...
    youtube_music: (
        <svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5">
            <path d="M12 0C5.376 0 0 5.376 0 12s5.376 12 12 12 12-5.376 12-12S18.624 0 12 0zm0 19.344c-4.053 0-7.344-3.291-7.344-7.344S7.947 4.656 12 4.656 19.344 7.947 19.344 12 16.053 19.344 12 19.344zm3.327-7.917l-4.707-2.717a.647.647 0 0 0-.969.563v5.432a.648.648 0 0 0 .969.563l4.707-2.716a.65.65 0 0 0 0-1.125z" />
        </svg>
    ),
...
export function AudioPreview({ data }: AudioPreviewProps) {
    ...
    // Update Context for Mobile Frame
    useEffect(() => {
        ...
        <div className="absolute inset-0 w-full h-full font-sans overflow-hidden bg-white text-slate-900 select-none" style={backgroundStyle}>
            ...
            <div className="relative w-full h-full overflow-y-auto no-scrollbar flex flex-col z-10">
                ...
                <div className="flex-none flex flex-col justify-center items-center pb-8 px-4 text-center">
                    ...
                    {/* Meta Outside */}
                    <h1 className="text-2xl font-bold text-slate-900 tracking-tight drop-shadow-sm mb-1 px-2 line-clamp-2">
                        {audioData.title}
                    </h1>
                        ...
                        <div className="space-y-3">
                            ...
                            {hasStreaming && (
                                <div className="grid grid-cols-4 gap-2 pt-2 border-t border-slate-100/50 mt-2">
                                    {Object.entries(audioData.streaming).map(([key, url]) => {
                                        if (!url) return null;
                                        const style = getPlatformStyle(key);
                                        const icon = PlatformIcons[key as keyof typeof PlatformIcons] || PlatformIcons.default;
                                        const label = key.replace('_', ' '); // simple fallback

                                        return (
                                            <a key={key} href={url as string} target="_blank" rel="noreferrer" className="flex items-center justify-center" title={label}>
