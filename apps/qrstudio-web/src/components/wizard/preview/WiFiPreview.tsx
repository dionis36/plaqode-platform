...
export function WiFiPreview({ data }: WiFiPreviewProps) {
    ...
    const network = {
        title: activeData?.network_info?.title || wifi.ssid || (hasUserInput ? '' : fallback.network_info.title),
    ...
    const getSecurityBadge = () => {
        switch (wifi.security) {
            case 'WPA2':
            case 'WPA3':
                return { label: wifi.security, color: '#F59E0B', bg: '#FFFBEB' }; // Yellowish/Amber for security
            case 'WEP':
            ...
            <div className="relative w-full h-full overflow-y-auto no-scrollbar flex flex-col z-10">
                ...
                <div className="flex-none flex flex-col justify-center items-center pb-8 px-4 text-center">
                    ...
                    <div className="relative group mb-5">
                        ...
                        >
                            {network.logo ? (
                                <img src={network.logo} alt="Logo" className="w-full h-full object-cover" />
                            ) : (
                                <Wifi
                                    className="w-10 h-10"
                    ...
                    <div className="w-full max-w-sm bg-white/60 backdrop-blur-3xl rounded-[2.5rem] shadow-[0_30px_60px_-10px_rgba(0,0,0,0.1)] border border-white/80 px-6 py-8 flex flex-col items-stretch animate-in slide-in-from-bottom-8 duration-700 ring-1 ring-white/40">
                        ...
                        <div className="space-y-6">
                            ...
                            <button
                                ...
                                style={{
                                    backgroundColor: primaryColor, // Or fixed yellow if requested? User said "CHECK IMG" but implied functionality update. Image had yellow. I'll stick to primaryColor for theming but ensure it looks good.
