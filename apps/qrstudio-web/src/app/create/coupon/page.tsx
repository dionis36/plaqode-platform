...
import { CouponForm } from '@/components/wizard/forms/CouponForm';
import { PhoneMockup } from '@/components/common/PhoneMockup';
import { CouponPreview } from '@/components/wizard/preview/CouponPreview';
...
function CouponPageContent() {
    ...
    // Use 'coupon' validation key (needs to be added to validation hook, or we make a simple check here)
    const { isValid } = useTemplateValidation('coupon');

    // Quick validation override for now until hook is updated
    const localIsValid = !!(payload.coupon?.title && payload.coupon?.code);
    ...
    const handleNext = () => {
        router.push(`/create/coupon/design${editId ? `?edit=${editId}` : ''}`);
    };
    ...
    return (
        ...
        >
            <div className="w-full px-3 sm:px-6 pb-20">
                <SEO
                    title="Create Coupon QR Code"
                    description="Create digital coupons and offers in minutes"
                />
