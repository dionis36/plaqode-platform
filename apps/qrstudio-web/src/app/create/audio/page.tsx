...
import { AudioForm } from '@/components/wizard/forms/AudioForm';
import { PhoneMockup } from '@/components/common/PhoneMockup';
import { AudioPreview } from '@/components/wizard/preview/AudioPreview';
...
function AudioPageContent() {
    ...
    // Use 'audio' validation key
    const { isValid } = useTemplateValidation('audio');

    // Quick validation override for now
    const localIsValid = !!(payload.audio?.title && payload.audio?.audio_url);
    ...
    const handleNext = () => {
        router.push(`/create/audio/design${editId ? `?edit=${editId}` : ''}`);
    };
    ...
    return (
        ...
        >
            <div className="w-full px-3 sm:px-6 pb-20">
                <SEO
                    title="Create Audio/MP3 QR Code"
                    description="Share music, podcasts, or audio guides instantly"
                />
