...
import { BusinessPageForm } from '@/components/wizard/forms/BusinessPageForm';
import { PhoneMockup } from '@/components/common/PhoneMockup';
import { BusinessPagePreview } from '@/components/wizard/preview/BusinessPagePreview';
...
function BusinessPageContent() {
    ...
    // Use 'business' validation key
    const { isValid } = useTemplateValidation('business');

    // Quick validation override for now
    const localIsValid = !!payload.business?.name;
    ...
    const handleNext = () => {
        router.push(`/create/business/design${editId ? `?edit=${editId}` : ''}`);
    };
    ...
    return (
        ...
        >
            <div className="w-full px-3 sm:px-6 pb-20">
                <SEO
                    title="Create Business Page QR Code"
                    description="Create a mini-website for your business with location, hours, and contacts"
                />
                        ...
                        <div className="sticky top-6 w-full flex flex-col items-center h-fit">
                            <div className="transform transition-all duration-500 origin-top scale-[0.85] xl:scale-[0.9]">
                                <PhoneMockup className="shadow-2xl shadow-slate-300/50">
                                    <BusinessPagePreview data={payload} />
                                </PhoneMockup>
                            </div>
                        </div>
                    </div>
